...
module.exports = {
  ...
  interval: 30000,
  enabled: process.env.FIM_ID != null,
  async execute(client) {
    ...
    do {
      ...
      let url = `${ openseaEventsUrl }?collection_slug=${ process.env.COLLECTION_SLUG_FIM }&event_type=successful&only_opensea=false&occurred_before=${ newTimestamp }${ next == null ? '' : `&cursor=${ next }` }`;
      try {
        ...
        data.asset_events.forEach(function (event) {
          if (event.asset) {
            ...
            client.channels.fetch(process.env.FIM_ID)
              .then(channel => {
