...
module.exports = {
  ...
  interval: 30000,
  enabled: process.env.NVC_ID != null,
  async execute(client) {
    ...
    do {
      ...
      let url = `${ openseaEventsUrl }?collection_slug=${ process.env.COLLECTION_SLUG_NVC }&event_type=successful&only_opensea=false&occurred_before=${ newTimestamp }${ next == null ? '' : `&cursor=${ next }` }`;
      try {
        ...
        data.asset_events.forEach(function (event) {
          if (event.asset) {
            ...
            client.channels.fetch(process.env.NVC_ID)
              .then(channel => {
