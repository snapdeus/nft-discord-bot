...
module.exports = function createListingsBot(channel_id, slug) {
  return {
    name: 'listing',
    ...
    async execute(client) {
      ...
      console.log('listings for ' + slug + ' at time ' + newTimestamp);
      let next = null;
      let newEvents = true;
      let settings = {
        method: "GET",
      ...
      do {
        let url = `${ openseaEventsUrl }?collection_slug=${ slug }&event_type=created&only_opensea=false&occurred_before=${ newTimestamp }${ next == null ? '' : `&cursor=${ next }` }`;
        try {
